...
### Vérification de l'autocorelation spatiale 
...
base_j25_year_numeric <-  base_j25 %>%
  mutate(annee = as.numeric(annee), 
         x_l93 = as.numeric(x_l93), 
         y_l93 = as.numeric(y_l93), 
         statut_presence = as.factor(statut_presence))
...
```{r}
summary(base_j25_year_numeric)
```

```{r}
plot(variogram(statut_presence ~ annee , locations = ~ x_l93 + y_l93, data = base_j25_year_numeric), col = "black")

```
```{r}
library(ggplot2)
ggplot(base_j25_year_numeric, aes(x_l93, y_l93, color = statut_presence)) + geom_point()
```
```{r}
ggplot(base_j25_year_numeric, aes(statut_presence)) + geom_histogram(bins = 30)
